...
function step(field: Field, calm: number): void {
	stepParticles({
		field,
		aspect: ASPECT,
		time: 0,
...
describe("stepParticles drift", () => {
	it("eases toward the drift target exactly as before at full calm", () => {
	...
	it("scales the pull toward the drift target by the calm", () => {
		const field = makeField();
		step(field, 0.25);
