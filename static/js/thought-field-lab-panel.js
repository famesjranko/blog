 ...
 * and its own readout only, so the sheet is not rebuilt under the
 * finger; one held at 0 by a toggle is dimmed.
 * @param {Ctx} ctx
 * @param {string} title
...
function swirlControls(ctx) {
	...
	const picker = radios("lab-style", STYLES, style, (value) =>
		applyAndRender(ctx, (s) => ({
			...s,
			style: value === "pattern" || value === "off" ? value : "galaxy",
		})),
	);
...
function controls(ctx) {
	...
	const picker = radios("lab-engine", ENGINES, engine, (value) =>
		applyAndRender(ctx, (s) => ({
			...s,
			engine: value === "liquid" ? "liquid" : "swirl",
		})),
	);
	const active =
		engine === "liquid"
